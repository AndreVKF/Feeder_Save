...
if __name__ == "__main__":
    ...
    # Exec Process NAV
    Fun_Exec = Try_ExecProcessNAV(Refdate=Refdate, Log_DirSrc=Log_DirSrc,
                                  Queries=Queries, SQL_Server_Connection=SQL_Server_Connection)

    if Fun_Exec:
        print(f"Exec ProcessNAV Refdate:{Refdate} at {Time_Now}")
    else:
        print(f"Failure to Exec ProcessNAV Refdate:{Refdate} at {Time_Now}")
